...
{
	std::cout << "Connecting to scanner at " << Nav->param.r2000_sens_ip << " ... ";
	if (driver.connect(Nav->param.r2000_sens_ip, 80))
		std::cout << "OK" << std::endl;
	else
	{
		std::cout << "FAILED!" << std::endl;
		std::cerr << "Connection to scanner at " << Nav->param.r2000_sens_ip << " failed!" << std::endl;
		return false;
	}

	driver.setScanFrequency(Nav->param.r2000_sens_smp_frq);
	driver.setSamplesPerScan(Nav->param.r2000_sens_smp_cnt);
...
int R2000::sample(void)
{
	...
	for (int idx = 0; idx < scandata.distance_data.size(); idx++)
	{
		...
		float ang = first_angle + Nav->param.r2000_sens_angstep * idx;
		float r = ang * A2R;
