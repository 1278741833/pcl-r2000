...
{
	unsigned long i;
	unsigned long crc32 = 0;//CRC32校验码初值
	unsigned long tabitem;

	while(aSize--)
	{
		tabitem=( crc32 >> 24 )^ *aData++;
		crc32 = ( crc32 << 8 ) ^ crctable32[tabitem];
	}
	return crc32;
}
#endif
unsigned long CRC_32(const unsigned char* buf, int nLength)
{
    if (nLength < 1)
        return 0xffffffff;
    ...
    {
        crc = crctable32[(crc ^ buf[i]) & 0xff] ^ (crc >> 8);  
    }
